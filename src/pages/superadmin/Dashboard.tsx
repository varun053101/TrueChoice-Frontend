...
export default function SuperAdminDashboard() {
    ...
    const [currentAdmin, setCurrentAdmin] = useState<UserInfo | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            // Get current admin
            const adminRes = await superadminAPI.getCurrentAdmin();
            if (adminRes.admin) {
                setCurrentAdmin({
                    ...adminRes.admin,
                    id: adminRes.admin._id || adminRes.admin.id,
                });
            }
    ...
    const handleMakeAdmin = async (user: UserInfo) => {
        ...
        try {
            ...
            toast({
                title: 'Admin Updated',
                description: `${user.fullName} is now the admin`,
            });
            loadData();
        } catch (err: any) {
            toast({
    ...
    const handleMakeSuperadmin = async (user: UserInfo) => {
        ...
        try {
            ...
            toast({
                ...
                description: `${user.fullName} is now the superadmin. Logging you out...`,
            });
            // Logout and redirect to login since user's role changed
            setTimeout(() => {
                logout();
                navigate('/login');
            }, 1500);
            ...
            toast({
                title: 'Failed to transfer superadmin',
                description: err?.response?.data?.error || err?.message || 'An error occurred',
                variant: 'destructive',
            });
            setActionLoading(null);
    ...
    const voterUsers = filteredUsers.filter(u => u.role === 'voter');
        ...
        <DashboardLayout>
            ...
            >
                <Card className="border-2 border-primary/20">
                    <CardHeader>
                        ...
                        <CardDescription>
                            The admin can create elections, manage candidates, and control voting
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {currentAdmin ? (
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    Promote users to admin or transfer superadmin ownership
                                </CardDescription>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => setConfirmDialog({ open: true, action: 'make-admin', user })}
                                            ...
                                            <Button
                                                variant="destructive"
                                                size="sm"
                                                onClick={() => setConfirmDialog({ open: true, action: 'make-superadmin', user })}
                                                disabled={actionLoading === user.id}
                                            >
                                                {actionLoading === user.id ? (
                                                    <Loader2 className="w-4 h-4 animate-spin" />
            ...
            <Dialog open={confirmDialog.open} onOpenChange={(open) => !open && setConfirmDialog({ open: false, action: null, user: null })}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {confirmDialog.action === 'make-admin' ? 'Promote to Admin?' : 'Transfer Superadmin Ownership?'}
                        </DialogTitle>
                        <DialogDescription>
                            {confirmDialog.action === 'make-admin' ? (
                                <>
                                    This will make <strong>{confirmDialog.user?.fullName}</strong> the new admin.
                                    The current admin will be demoted to voter.
                                </>
                            ) : (
                                <>
                                    <span className="text-destructive font-medium">Warning: This action cannot be undone!</span>
                                    <br /><br />
                                    You will transfer your superadmin privileges to <strong>{confirmDialog.user?.fullName}</strong>.
                                    You will be demoted to voter and lose all admin access.
                                </>
                            )}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (confirmDialog.action === 'make-admin' && confirmDialog.user) {
                                    handleMakeAdmin(confirmDialog.user);
                                } else if (confirmDialog.action === 'make-superadmin' && confirmDialog.user) {
                                    handleMakeSuperadmin(confirmDialog.user);
                                }
                            }}
